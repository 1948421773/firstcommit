...
import com.musicdo.musicshop.util.CountDownTimer;
...
public class ModifyPasswordActivity extends BaseActivity implements View.OnClickListener, View.OnFocusChangeListener, CompoundButton.OnCheckedChangeListener {
    ...
    private TextView tv_regist_Code,tv_title;
    ...
    private LinearLayout ll_regist_getcode, ll_regist_setting_password,ll_back;
...
//            tv_regist_Code.setText(millisUntilFinished / 1000 + "秒");
        ...
        switch (v.getId()) {
            ...
            case R.id.bt_confirm_password_submit:
                ...
                if (et_confirm_password.getText().toString().trim().equals("")){
                    ToastUtil.showShort(context, "请输再次入密码");
                    return;
                }
                if (et_new_password.getText().toString().trim().length()<6){
                    ToastUtil.showShort(context, "密码不能小于6位字符");
                    return;
                }
    ...
    private void getVerificationCode(String phoneNumber) {
        OkHttpUtils.post(AppConstants.GetVerificationCode)
                .tag(this)
