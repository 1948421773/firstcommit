...
public class ProductCollectListActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_productcollectlist);
        ...
        switch (v.getId()){
            ...
            case R.id.tv_right_one_bt:
                if(productCollectBeans==null){
                    ...
                    return;
                }else if(productCollectBeans.size()==0){
                    ToastUtil.showShort(context,"暂无数据");
                    return;
                }
                CustomDialog.Builder builderAll= new CustomDialog.Builder(context);
                builderAll.setMessage("确定删除所有收藏?");
                builderAll.setTitle("");
                builderAll.setNegativeButton("确定", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        StringBuffer paramStrBuff = new StringBuffer("");
                        for (ProductCollectBean CollectBeans : productCollectBeans) {
                            paramStrBuff.append(CollectBeans.getID() + ",");
                        }
                        DeleteCollectProduct(paramStrBuff.substring(0, paramStrBuff.length() - 1));
                        dialog.dismiss();
                    }
                });
                builderAll.setPositiveButton("取消", new DialogInterface.OnClickListener() {
