...
/**
 * 描述:
 * 作者：haiming on 2017/8/3 17:32
 * 邮箱：1948421773@qq.com
 * 版 本 ：1.0
 * 备 注 ：
 */

public class SpecSizeGridAdapter extends RecyclerView.Adapter<SpecSizeGridAdapter.MyViewHolder> implements View.OnClickListener {
    private Context mContext;
    View rootView;
    SpecSizeGridAdapter.OnSearchItemClickListener mOnItemClickListener;
...
//适配器初始化
    ...
    public SpecSizeGridAdapter(Context context,ArrayList<SpecitemDataBean> datas){
        mContext=context;
        this.datas=datas;
    ...
    @Override
    public SpecSizeGridAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //根据item类别加载不同ViewHolder
        View view= inflater.inflate(R.layout.item_spec_color, parent, false);//这个布局就是一个imageview用来显示图片
        return new SpecSizeGridAdapter.MyViewHolder(view);

    }

    @Override
    public void onBindViewHolder(SpecSizeGridAdapter.MyViewHolder holder, int position) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.cb_spec_color_chekbox:
                ...
                checkInterface.checkChild(1,(int) v.getTag());// 暴露组选接口,尺寸组下标为1
                break;
        }
    }

    public void setOnItemClickListener(SpecSizeGridAdapter.OnSearchItemClickListener listener){
