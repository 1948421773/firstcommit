...
public class EditAddressActivity extends BaseActivity implements View.OnClickListener,TextView.OnEditorActionListener{
    ...
    private TextView tv_right_one_bt,tv_default_address,tv_address_areas,tv_delete_address,tv_title;
    ...
    protected void onResume() {
        ...
        if(addressBean!=null){
            ...
            CountyID=addressBean.getCountyID();
        }
...
//                    Toast.makeText(JsonDataActivity.this,"Parse Succeed",Toast.LENGTH_SHORT).show();
                    ...
                    if(isLoaded) {
                        for (AreasBean o1:options1Items){
                            if (o1.getProvinceId().equals(addressBean.getProvinceID())){
                                paramStrBuff.append(o1.getProvinceName());
                                ProvinceName=o1.getProvinceName();
                                for (int i = 0; i <o1.getDistrict().size() ; i++) {
                                    if (o1.getDistrict().get(i).getDistrictId().equals(addressBean.getCityID())){
                                        paramStrBuff.append(o1.getDistrict().get(i).getDistrictName());
                                        CityName=o1.getDistrict().get(i).getDistrictName();
                                        for (int j = 0; j < o1.getDistrict().get(i).getCounty().size(); j++) {
                                            if (o1.getDistrict().get(i).getCounty().get(j).getCountyId().equals(addressBean.getCountyID())){
                                                paramStrBuff.append(o1.getDistrict().get(i).getCounty().get(j).getCountyName());
                                                CountyName=o1.getDistrict().get(i).getCounty().get(j).getCountyName();
                                                break;
                                            }
                                        }
                                    }
                        ...
                        tv_address_areas.setText(paramStrBuff.toString());
                    }else{
...
//                    Toast.makeText(JsonDataActivity.this,"Parse Failed",Toast.LENGTH_SHORT).show();
    ...
    private void initView() {
        ...
        tv_delete_address.setOnClickListener(this);
...
//                    tv_default_address.setTextColor(getResources().getColor(R.color.text_black));
    ...
    private void showAddress() {
        if (addressBean!=null){
            ed_address_name.setText(addressBean.getName());
            ed_address_phone.setText(addressBean.getMoblie());
            tv_address_areas.setText("");
            ed_address_detail_address.setText(addressBean.getAddress());
        ...
        switch (v.getId()){
            case R.id.tv_right_one_bt:
                ...
                if (address_detail_address!=""){
                    ed_address_detail_address.setSelection(address_detail_address.trim().length());
                }
                if (address_detail_address.trim().length()>36){
                    address_detail_address=address_detail_address.trim().substring(0,36);
                }

                if(address_name.trim().length()!=0&&ed_address_phone.getText().toString().trim().length()!=0&&tv_address_areas.getText().toString().trim().length()!=0&&ed_address_detail_address.getText().toString().trim().length()!=0){
                    if (SpUtils.isMobileNO(ed_address_phone.getText().toString())){
                        EditUserAddress(ed_address_name.getText().toString().trim(),ed_address_phone.getText().toString(),address_detail_address,isDefault,ProvinceID,CityID,CountyID);
                    }else{
                        ToastUtil.showShort(context,"手机号码不正确");
                    }
                }else if(ed_address_name.getText().toString().trim().length()==0){
                ...
                }else if(ed_address_detail_address.getText().toString().trim().length()==0){
...
//                    mHandler.sendEmptyMessage(MSG_LOAD_DATA);
    ...
    private void doDelete() {
        if (addressBean.getID()==0)
            return;
        OkHttpUtils.post(AppConstants.DeleteUserAddress)
                .tag(this)
    ...
    private void EditUserAddress(String name,String PhoneNumber,String Address,String IsDefault,String ProvinceID,String CityID,String CountyID) {
        if (addressBean.getID()==0)
            return;
        OkHttpUtils.post(AppConstants.EditUserAddress)
                .tag(this)
                .params("ID",addressBean.getID())
                .params("UserID",AppConstants.USERID)
                .params("Name",name)
