...
import com.musicdo.musicshop.activity.SubmitOrderActivity;
...
public class ShoppingCartFragment extends BaseFragment implements ShopcartExpandableListViewAdapter.CheckInterface,RefreshCompleteCallBack,ShopcartExpandableListViewAdapter.ModifyCountInterface, View.OnClickListener,ShopcartExpandableListViewAdapter.DeleteInterface{
...
//                expanedAll();
    ...
    public View initView() {
        View view = LayoutInflater.from(mContext).inflate(R.layout.main_shoppingcart_layout, null);
//        virtualData();
        dialog = new LoadingDialog(mContext,R.style.LoadingDialog);
        exListView = (SExpandableListView) view.findViewById(R.id.exListView);
        cb_check_all = (CheckBox) view.findViewById(R.id.all_chekbox);
        tv_total_price = (TextView) view.findViewById(R.id.tv_total_price);
        tv_add_keep = (TextView) view.findViewById(R.id.tv_add_keep);
        tv_cart_calculate = (TextView) view.findViewById(R.id.tv_cart_calculate);
//        tv_empty = (TextView) view.findViewById(R.id.tv_empty);
        tv_cart_complete_edit = (TextView) view.findViewById(R.id.tv_cart_complete_edit);
...
//                                return;
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            PropertysText=Cace[i];
                            break;
                        case 3:
                            price=Double.valueOf(Cace[i]);
                            break;
                        case 4:
                            ProductID=Integer.valueOf(Cace[i]);
                            break;
                        case 5:
                            Count=Integer.valueOf(Cace[i]);
                            break;
                    }
                }
                if (!Guid.equals("")){
...
//                        }
    ...
    private void setData() {
        if (selva==null) {
            selva = new ShopcartExpandableListViewAdapter(groupCarts, childCarts, mContext, specItemBeans);
        ...
        switch (v.getId())
        {
            ...
            case R.id.tv_cart_calculate:{//删除或者结算
                if(allEdit){//删除
                    if (totalCount == 0)
                    {
                        Toast.makeText(mContext, "请您选择商品", Toast.LENGTH_LONG).show();
                        return;
                    }
            ...
            case R.id.tv_add_keep://加入收藏夹
                if (totalCount == 0)
                {
                    Toast.makeText(mContext, "请选择宝贝", Toast.LENGTH_LONG).show();
                    return;
                }
                StringBuffer sb=new StringBuffer("");
