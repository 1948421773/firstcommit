...
public class PurchasePiano extends BaseFragment implements View.OnClickListener {
    View rootView;
    private final static String TAG = "PurchasePiano";
    PurchaselistPianoAdapter purchaselistPianoAdapter;
    int page=3;
...
//                            Toast.makeText(mContext, "点击了"+position, Toast.LENGTH_SHORT).show();
    ...
    private void setData() {
        if(purchaselistPianoAdapter==null){
            purchaselistPianoAdapter = new PurchaselistPianoAdapter(mContext,meizis);
            rc_popu_tend.setAdapter(purchaselistPianoAdapter);//recyclerview设置适配器
            purchaselistPianoAdapter.setOnItemClickListener(new PurchaselistPianoAdapter.OnRecyclerViewItemClickListener() {
                @Override
                public void onItemClick(View view, int position) {
//                            Toast.makeText(mContext, "点击了"+position, Toast.LENGTH_SHORT).show();
                    Intent intent=new Intent(mContext,ProductDetailActivity.class);
